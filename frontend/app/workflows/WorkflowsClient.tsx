...
function buildPreviewSections(
  ...
  for (const ws of workflowSteps) {
    ...
    const fromExec =
      stepInput?.config?.preview_title_map ??
      stepInput?.preview_title_map ??
      stepInput?.config?.config?.preview_title_map;
    const execPreviewTitleMap = coercePreviewTitleMap(fromExec);
    const agentId = (execStep as any)?.agent_id ?? (ws as any)?.agent_id;
    const fromWcs = agentId ? (workflowWcs as any)?.[agentId]?.preview_title_map : undefined;
    const wcsPreviewTitleMap = coercePreviewTitleMap(fromWcs);
...
export function WorkflowsClient() {
  ...
  const openPanel = async (wf: Workflow) => {
    ...
    try {
      const exec = await getWorkflowLatestExecution(wf.id);
      setSelectedLatestExecution(exec);

      if (exec) {
        const [execSteps, wfSteps] = await Promise.all([
          listExecutionSteps(exec.id).catch(() => []),
          listWorkflowSteps(wf.id).catch(() => []),
        ]);
        setSelectedExecutionSteps((execSteps ?? []).slice());
        setSelectedWorkflowSteps((wfSteps ?? []).slice().sort((a, b) => a.step_number - b.step_number));
      }
    } catch {
      // ignore
    }
  ...
  return (
    <>
                    ...
                    <td className="p-4 align-top text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Link
                          ...
                          className={`size-8 flex items-center justify-center rounded-md hover:bg-[#282b39] transition-colors ${
                            latest
      ...
      <div
        className={`fixed inset-y-0 right-0 w-1/2 min-w-[520px] bg-surface-dark border-l border-border-dark shadow-2xl z-30 transform transition-transform duration-300 ease-out grid grid-rows-[auto,1fr] ${
      ...
      >
        ...
        <div className="overflow-y-auto p-6 flex flex-col gap-4">
            ...
            <button
              ...
              className={`h-8 rounded-lg px-3 text-xs font-semibold transition-colors border ${
                panelTab === "preview"
                  ...
                  : "bg-[#282b39] text-white border-transparent hover:border-border-dark"
              }`}
            >
              Preview
            </button>
          ...
          {selectedWorkflow && selectedLatestExecution && (
            <div className="rounded-xl border border-border-dark bg-[#111218] p-4">
                  ...
                  ? JSON.stringify(
                      {
                        ...
                        latest_execution: selectedLatestExecution,
